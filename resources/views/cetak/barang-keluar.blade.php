...
@section('content')
    <div class="text-center report-title">LAPORAN BARANG Keluar</div>
    ...
    <table id="my-table" class="table display" style="margin-top: 10px">
        ...
        <tr>
            ...
            <th width="" class="text-center f12 text-body-small">No. Keluar</th>
            <th width="" class="f12 text-body-small">Customer</th>
            <th width="" class="f12 text-body-small">No. Hp</th>
        ...
        @foreach($data as $v)
            <tr>
                ...
                <td class="text-center text-body-small">{{ $v->barang_keluar->tanggal }}</td>
                <td class="text-center text-body-small">{{ $v->barang_keluar->no_keluar }}</td>
                <td class="text-body-small">{{ $v->barang_keluar->nama }}</td>
                <td class="text-body-small">{{ $v->barang_keluar->no_hp }}</td>
                ...
                <td class="text-body-small">{{ $v->barang_keluar->keterangan }}</td>
            </tr>
        @endforeach
        </tbody>
    </table>
